...
class HostExecutor implements Executor {
	async exec(command: string, options?: ExecOptions): Promise<ExecResult> {
		...
		return execProcess(shell, [...shellArgs, command], options);
	}
...
class DockerExecutor implements Executor {
	...
	async exec(command: string, options?: ExecOptions): Promise<ExecResult> {
		return execProcess("docker", ["exec", this.container, "sh", "-c", command], options);
	}
...
function execProcess(cmd: string, args: string[], options?: ExecOptions): Promise<ExecResult> {
	return new Promise((resolve, reject) => {
